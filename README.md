...
- **发送留言**：玩家可以自由地向组或者玩家留言，支持 MC 的颜色代码，以及自动将网址转换为可被点击的文字。
...
2. 非玩家使用 `!!div join <group> [<player_id>]` 时，插件会让 `server` 加入组
...
插件会将所有以 `http` 开头，以空格或字符串结尾的内容识别为网址
